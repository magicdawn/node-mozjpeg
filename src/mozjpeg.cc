...
EncodeResult encode(uint8_t *image_in, int image_width, int image_height,
                    MozJpegOptions opts)
{
...
void inline encodeFinalizeCallback(Env env, uint8_t *data) { delete[] data; }
...
Value BindEncode(const CallbackInfo &info)
{
  ...
  EncodeResult encodeResult =
      encode(buf.Data(), width, height, getMozjpegOptions(options));
